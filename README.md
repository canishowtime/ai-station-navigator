...
**AI Station Navigator** 是一款基于 Claude Code 引擎构建的模块化 AI 工作站。它模仿计算机底层调度逻辑，通过独创的“总线与调度”机制，将繁杂的 AI 任务精准路由至隔离的子智能体（Sub-Agents）执行。项目集成了沙盒化执行环境与“应用商店式”技能管理，配合全绿色的免安装运行环境，旨在为开发者提供一个“解压即用”、性能恒定、无限扩展的个人 AI 智慧中枢。
...
| 物理组件 | 软件映射 | 角色功能描述 |
| --- | --- | --- |
| **中央处理器 (CPU)** | **Claude Code + LLM + CLAUDE.md** | **核心逻辑层**：负责意图识别、指令调度、任务拆解与上下文管理。 |
...
| **集成环境 (Runtime)** | **Portable Environment** | **底层支撑**：预集成绿色版 Python, Node.js, Git，确保环境高度统一，增强潜在扩展能力。 |
...
* **自动化 Hooks**：启动时自动轮转日志、交付时自动清理沙盒、定时同步技能状态。


* 🛡️ **沙盒化执行环境**
* 严格权限控制：内核只读（`bin/`, `docs/`），仅允许子智能体在 `mybox/` 写入。
* 安全隔离：防止 AI 生成的实验性代码误删系统文件。
...
2. **就绪**：按照屏幕提示去人安装缺失组件并输入 `LLM-API-KEY`，即可进入启动状态。

### 2. 智能化管理 (对话即指令)

直接在对话框输入以下指令，通过 **Sub-Agent** (子进程) 实现技能管理与运行，有效减少主上下文污染，git链接已配置网络加速器，解决源码获取网络问题：
