...
2. **执行命令**: 使用 `Bash` 工具执行
   - **[P0-FORCE] 严禁使用 `run_in_background=true`，必须同步执行**
   - 超时限制: 90秒
...
| **绝对禁止** | **后台运行** | **[P0-FORCE] 严禁使用 run_in_background=true，必须同步执行** |
...
```
1. [Call Skill]
   └─ 使用 Skill 工具调用 markdown-converter
      → Skill 工具自动处理寻址、解析、执行
